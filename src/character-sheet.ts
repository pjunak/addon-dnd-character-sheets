...
export function featDetails(projection: Projection | undefined, locale: string): HTMLElement | undefined {
  ...
  const t = translator(locale), root = panel(t("Feats")); root.className = "dse-section";
...
export function abilityRail(view: SheetView): HTMLElement {
  ...
  for (const ability of abilities) {
    ...
    const proficiency = styled("span", "dse-dot dse-shield"); proficiency.append(icon("M12 2.4 19.3 5.3V11c0 4.8-3.3 8.6-7.3 10.5C8 19.6 4.7 15.8 4.7 11V5.3Z")); proficiency.dataset["proficient"] = String(save["proficient"] === true); proficiency.title = t("{0} — saving throw: {1}", [t(abilityNames[ability]!), t(save["proficient"] === true ? "Proficient" : "Untrained")]); proficiency.setAttribute("role", "img"); proficiency.setAttribute("aria-label", proficiency.title);
...
export function vitals(view: SheetView): HTMLElement {
  ...
  for (const slot of ["armor", "shield", "worn", "attuned"] as const) {
    ...
    for (const item of items) {
      ...
      if (slot === "attuned" && item.location !== "equipped") token.append(styled("span", "dse-equipment-location", t(label(item.location))));
      if (view.editing) { const remove = button("×", () => { if (slot === "attuned") attuneEquipment(item, false, view.equipment); else moveEquipment(view.input.play.inventory, item.id, "carried", view.equipment, view.projection); view.change(); view.refresh(); }); remove.setAttribute("aria-label", slot === "attuned" ? t("Unattune {0}", [item.name]) : t("Remove {0}", [item.name])); token.append(remove); }
      group.append(token);
    }
...
export function combatDetails(view: SheetView): HTMLElement {
  ...
  for (const key of ["resistances", "damageImmunities", "conditionImmunities"]) if (sheet[key] !== undefined) traits.append(el("p", el("strong", t(label(key)) + ": "), human(sheet[key])));
  ...
  const feats = featDetails(view.projection, view.locale); if (feats) root.append(feats);
  return root;
}
