...
export class CharacterClient {
  ...
  async call(method: string, request: Omit<Request, "contractVersion">): Promise<Response> {
    const result = await this.service.call<Response>(method, { ...request, contractVersion: "character.v1" }, { providerAddonId: this.providerId, deadlineMs: 30000, signal: this.signal });
    if (result.contractVersion !== "character-response.v1" || result.key !== request.key) throw new Error("The character service returned an incompatible response.");
    return result;
  }
...
export interface Draft { version: 1; baseRevision: number; inputs: Inputs; updatedAt: string }
export class DraftStore {
  readonly key: string;
  constructor(actor: string, character: string, private readonly storage: Storage = localStorage) { this.key = `dnd-character-draft.v1:${location.origin}:${actor}:${character}`; }
  read(): Draft | undefined {
    const body = this.storage.getItem(this.key); if (body === null) return undefined;
    const value = JSON.parse(body) as Draft;
    if (value.version !== 1 || !Number.isSafeInteger(value.baseRevision) || !value.inputs?.build || !value.inputs.play) throw new Error("The saved draft cannot be read. Export it before clearing local storage.");
    return value;
  }
...
export function exportCharacter(state: State, history?: ExternalRevision[]): string {
  ...
  if (history && (history.length > 5 || new TextEncoder().encode(body).length > transferLimit)) throw new Error("This history archive exceeds the transfer limit. Export fewer revisions; full installation backups retain all history.");
  return body;
}
...
export class TransferStore {
  readonly key: string;
  constructor(actor: string, character: string, private readonly storage: Storage = localStorage) { this.key = `dnd-character-transfer.v1:${location.origin}:${actor}:${character}`; }
  read(): string { return this.storage.getItem(this.key) ?? ""; }
  write(body: string): void { parseCharacter(body); this.storage.setItem(this.key, body); }
  clear(): void { this.storage.removeItem(this.key); }
}
export function externalHistory(body: string): ExternalRevision[] {
  ...
  return value as ExternalRevision[];
}
export function parseCharacter(body: string): Inputs {
  if (new TextEncoder().encode(body).length > 1000000) throw new Error("This character file exceeds the import limit.");
  const envelope = object(JSON.parse(body));
  if (envelope["format"] !== "dnd-character.v1" || envelope["schemaVersion"] !== "4.0.0" || Object.keys(envelope).some(key => !["format", "schemaVersion", "inputs", "savedRules", "savedProjection", "externalHistory"].includes(key))) throw new Error("Choose a current dnd-character.v1 export. Retired sheets and raw objects are not supported.");
  ...
  return input as unknown as Inputs; // Full closed-schema validation is server-owned.
}
