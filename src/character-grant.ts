...
export function grantForm(input: Inputs, evaluation: Result | undefined, feats: readonly CatalogRecord[], previous: Grant | undefined, submit: (grant: Grant) => void | Promise<void>, locale = "en"): Node[] {
  ...
  const renameEffects = (): void => { [...list.children].forEach((row, index) => { row.querySelector("h3")!.textContent = t("Effect {0}: {1}", [index + 1, t(label(grant.effects[index]!.target))]); }); };
  const appendEffect = (effect: Effect): HTMLSelectElement => {
    const row = panel(""), identity = "grant-effect-" + crypto.randomUUID(), keySlot = el("div");
    const heading = row.querySelector("h3")!; heading.id = identity;
    row.setAttribute("role", "group"); row.setAttribute("aria-labelledby", identity);
    const ownedField = (name: string, control: HTMLElement): HTMLElement => {
      const wrapper = field(t(name), control); wrapper.dataset["uiKey"] = identity + ":" + name; return wrapper;
    };
    ...
    const renderKey = (): void => {
      ...
      if (["abilityScore", "abilityCap", "savingThrow", "proficiency", "resourceMax", "sense"].includes(effect.target)) {
        ...
        key.required = true; keySlot.append(ownedField("Applies to", key));
      }
    };
    ...
    target.required = true; mode.required = true; amount.required = true;
    row.append(ownedField("Effect", target), keySlot, ownedField("Operation", mode), ownedField("Amount", amount),
      button(t("Remove effect"), () => {
        const index = grant.effects.indexOf(effect);
        grant.effects.splice(index, 1); row.remove(); renameEffects();
        const next = list.children[index] ?? list.children[index - 1];
        (next?.querySelector("select") ?? add).focus();
      }));
    renderKey(); list.append(row); renameEffects(); return target;
  };
  const add = button(t("Add effect"), () => {
    const effect: Effect = { target: "maxHp", mode: "add", value: 0 };
    grant.effects.push(effect); appendEffect(effect).focus();
  });
  ...
  for (const effect of grant.effects) appendEffect(effect);
  ...
  const level = numberInput(grant.effectiveLevel, value => { grant.effectiveLevel = value ?? 1; }, 1); level.required = true;
  const condition = select(grant.condition, ["always", "equipped", "attuned"].map(id => ({ id, label: t(label(id)) })), value => { grant.condition = value; }, t); condition.required = true;
  const apply = button(t("Apply DM grant"), () => undefined); apply.type = "submit";
  const form = el("form",
    field(t("Name"), name), field(t(previous ? "Reason for amendment" : "Reason"), reason),
    field(t("Effective character level"), level), effects,
    field(t("Granted feat (optional)"), combo(grant.feat?.id ?? "", feats.map(record => ({ id: record.id, label: String(record.value["name"] ?? record.id) })), value => { if (value) grant.feat = { kind: "feat", id: value }; else delete grant.feat; }, t)),
    ...
    field(t("Condition"), condition),
    field(t("Item instance"), combo(grant.itemId ?? "", input.play.inventory.map(item => ({ id: item.id, label: item.name })), value => { if (value) grant.itemId = value; else delete grant.itemId; }, t)),
    field(t("Expires at (RFC 3339, optional)"), textInput(grant.expiresAt ?? "", value => { grant.expiresAt = value; })), apply);
