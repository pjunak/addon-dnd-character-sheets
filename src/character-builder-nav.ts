...
export function builderShell(view: BuildView, state: BuilderNavigation, body: HTMLElement, navigate: (tab: string, target?: string) => void): HTMLElement {
  ...
  const progress = styled("progress", "dse-build-meter"); progress.max = Number(guidance["total"]) || 1; progress.value = Number(guidance["complete"]) || 0; progress.setAttribute("aria-label", t("Builder completion"));
  ...
  for (const section of rows(guidance["sections"])) {
    ...
    for (const issue of rows(section["issues"])) step.append(button((issue["repair"] ? t("Review choice: {0}", [builderLabel(issue, view.locale)]) : builderLabel(issue, view.locale)) + " →", () => navigate(String(issue["tab"]), String(issue["id"]))));
    rail.append(step);
  }
  ...
  if (next) {
    ...
    action.className = "dse-builder-next"; main.append(action);
  }
...
export function focusBuilderTarget(root: HTMLElement, target: string): void {
  const node = root.querySelector<HTMLElement>('[data-builder-target="' + CSS.escape(target) + '"]')
    ?? root.querySelector<HTMLElement>("#character-choice-" + CSS.escape(encodeURIComponent(target)));
  if (!node) return;
  root.querySelector<HTMLElement>(".dnd-builder-tabs [aria-selected=true]")?.scrollIntoView({ block: "nearest", inline: "nearest", behavior: "instant" });
  const pending = node.querySelector<HTMLElement>("[data-builder-pending]") ?? node;
  for (let parent: HTMLElement | null = pending; parent && parent !== root; parent = parent.parentElement) if (parent instanceof HTMLDetailsElement) parent.open = true;
  const control = [...pending.querySelectorAll<HTMLElement>("input,select,textarea,button")]
    .find(element => !element.matches(":disabled,[hidden]") && element.getClientRects().length > 0 && getComputedStyle(element).visibility !== "hidden");
  const focus = control ?? pending; if (!control) focus.tabIndex = -1;
