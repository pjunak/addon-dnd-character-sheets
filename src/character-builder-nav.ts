...
export function builderShell(view: BuildView, state: BuilderNavigation, body: HTMLElement, navigate: (tab: string, target?: string) => void): HTMLElement {
  ...
  rail.setAttribute("aria-label", t("Builder progress")); rail.addEventListener("toggle", () => { state.open = rail.open; });
  ...
  for (const section of rows(guidance["sections"])) {
    ...
    for (const issue of rows(section["issues"])) step.append(button(String(issue["label"]) + " →", () => navigate(String(issue["tab"]), String(issue["id"]))));
    rail.append(step);
  }
  ...
  const nav = tabStrip(t("Builder sections"), tabs, state.tab, id => navigate(id), "dnd-builder"); nav.classList.add("dnd-builder-tabs");
  const derived = object(view.evaluation?.sheet["derived"]), summary = styled("div", "dse-builder-summary");
  for (const [name, key] of [["Maximum HP", "maxHp"], ["Armor class", "armorClass"], ["Proficiency", "proficiencyBonus"]]) summary.append(styled("div", "codex-tile", styled("span", "dse-stat-label", t(name!)), el("strong", human(derived[key!]))));
