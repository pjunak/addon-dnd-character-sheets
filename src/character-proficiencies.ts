...
interface TrainingEntry { name: string; reference?: Reference; path?: string }
interface TrainingGroup { id: string; name: string; available: boolean; entries: TrainingEntry[] }
...
export function proficiencyGroups(projection: Projection | undefined, locale: string): TrainingGroup[] {
  const t = translator(locale), training = object(projection?.sheet["proficiencies"]);
  const entry = (kind: string, id: string, path?: string): TrainingEntry => {
  ...
  const dictionary = (id: string, name: string, raw: unknown, accepts: (value: unknown) => boolean, kind: string, prefix: string): TrainingGroup => ({
  ...
  const list = (id: string, name: string, kind: string, raw: unknown): TrainingGroup => ({
    id, name: t(name), available: Array.isArray(raw),
    entries: [...new Set(Array.isArray(raw) ? raw.filter((value): value is string => typeof value === "string" && value.trim().length > 0) : [])].map(value => entry(kind, value)),
  });
  return [
...
export function proficiencyDetails(projection: Projection | undefined, locale: string): HTMLElement {
  ...
  for (const group of groups) {
    ...
    } else contents.append(el("span", t(group.available ? "None recorded" : "Not recorded")));
    const row = styled("div", "dse-training-group", heading, contents);
    row.dataset["training"] = group.id; values.append(row);
  }
  root.append(values); return root;
}
