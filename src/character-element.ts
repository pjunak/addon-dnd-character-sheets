...
export function defineCharacterElement(generation: string, client: CharacterClient): string {
  ...
  class CharacterElement extends HTMLElement {
    ...
    #syncBusyButtons(): void {
      for (const control of this.querySelectorAll<HTMLButtonElement>("button")) {
    ...
    async #load(): Promise<void> {
      ...
      await this.#guard(async () => {
        ...
        if (results.some(result => result.status === "rejected")) this.#message = "Some source catalogs could not be loaded. The saved character and draft remain available; reload to retry.";
      });
    }
    #publish(): void { this.#context?.edits.set({ dirty: this.#dirty, saving: this.#busy }); }
    async #refreshSaved(): Promise<void> {
      if (this.#busy || !this.#response) return;
      try {
        const response = await this.#call("load", { key: this.#key });
        if (response.revision === this.#response.revision) return;
        this.#response = response;
        if (this.#dirty) { this.#message = "The saved character changed. Your draft is preserved; compare and rebase it before saving."; this.#status(); }
        else { this.#input = structuredClone(response.state?.inputs ?? blank()); this.#baseRevision = response.revision; this.#evaluation = response.evaluation; this.#message = "The latest saved character revision was loaded."; this.#render(); }
      } catch { if (this.isConnected) { this.#message = "Could not refresh the saved character. Your current view and draft remain available."; this.#status(); } }
    }
    #changed = (): void => {
      this.#dirty = true; this.#publish();
      try { this.#draft?.write(this.#input, this.#baseRevision); this.#message = "Draft saved on this device. Character history changes only after review and save."; }
    ...
    async #evaluate(render: boolean): Promise<void> {
      ...
      try {
        ...
        if (epoch !== this.#epoch || !this.isConnected || JSON.stringify(inputs) !== JSON.stringify(this.#input)) return;
        this.#evaluation = response.evaluation; if (response.message) this.#message = response.message;
        if (render) this.#render(); else { this.#status(); this.#updateEvaluation(); }
      } catch (error) { this.#message = error instanceof Error ? error.message : "Rules preview failed; the draft is retained."; this.#status(); }
    }
    #view(): BuildView { return { locale: this.#context?.host.locale ?? "en", input: this.#input, evaluation: this.#evaluation, policy: this.#response?.policy ?? {}, catalogs: this.#catalogs, changed: this.#changed, refresh: () => { this.#render(); void this.#evaluate(true); } }; }
    #updateEvaluation(): void { const node = this.querySelector<HTMLElement>("[data-character-evaluation]"); if (node) node.replaceChildren(this.#issues(), ...(this.#evaluation ? [projectionView(this.#evaluation, this.#context?.host.locale ?? "en")] : [])); }
    #issues(): HTMLElement {
      const root = panel(this.#t("Rules review")), issues = this.#evaluation?.issues ?? [];
      if (this.#evaluation?.sheet["status"] === "needs-choices") { const list = el("ul"); for (const message of [Object.keys(this.#input.build.baseScores).length !== 6 && "Assign the six base ability scores.", !this.#input.build.species && "Choose a species.", !this.#input.build.background && "Choose a background.", !this.#input.build.levels.length && "Add your first class level."]) if (message) list.append(el("li", this.#t(message))); root.append(list); return root; }
      if (!issues.length) root.append(el("p", this.#t(this.#evaluation?.ready ? "All required choices and bounds are satisfied." : "Compatible rules are required to validate changes.")));
      const list = el("ul"); for (const issue of issues) { const item = el("li", this.#t("{0}{1}", [issue.severity === "blocker" ? this.#t("Needs attention: ") : "", issue.message])); if (this.#response?.role === "dm") item.append(el("details", el("summary", this.#t("Adjudication reference")), el("code", issue.id))); if (issue.reference) item.append(rule(this.#t("Rule details"), issue.reference)); list.append(item); } root.append(list); return root;
    }
    #render(): void {
    ...
    #spellChoices(): HTMLElement {
      ...
      for (const classOptions of rows(options["classes"])) {
        ...
        if (caster["prepares"] === "spellbook") root.append(picks(this.#t("{0} spellbook · {1} gained from levels", [this.#t(label(id)), human(caster["spellbookKnown"])]), leveled, this.#input.build.spells.spellbook[id] ?? [], value => { this.#input.build.spells.spellbook[id] = value; }));
      ...
      for (const choice of rows(options["pendingChoices"])) { const key = String(choice["key"]); root.append(picks(`Granted spells · choose ${human(choice["choose"])}`, strings(choice["eligibleSpellIds"]), this.#input.build.spells.grantChoices[key] ?? [], value => { this.#input.build.spells.grantChoices[key] = value; })); }
    ...
    #grants(): HTMLElement {
      ...
      for (const grant of this.#response?.state?.inputs.grants ?? []) { root.append(el("p", this.#t("{0} · {1} · {2} · {3} · {4}", [grant.name, this.#t(grant.active ? "active" : "revoked"), grant.reason, grant.actorId, grant.grantedAt]))); if (grant.active && this.#response?.role === "dm") root.append(button(this.#t("Amend {0}", [grant.name]), () => this.#grant(grant), !this.#editable), button(this.#t("Revoke {0}", [grant.name]), () => this.#review({ ...this.#base("revoke-grant"), grantId: grant.id, inputs: structuredClone(this.#input), summary: `Revoke DM grant: ${grant.name}` }), !this.#editable)); }
    ...
    #grant(previous?: Grant): void {
      this.#open(previous ? this.#t("Amend {0}", [previous.name]) : this.#t("DM given"), grantForm(this.#input, this.#evaluation, this.#catalogs.get("feat") ?? [], previous, grant => {
        this.#dialog?.close(); return this.#review({ ...this.#base(previous ? "amend-grant" : "grant"), inputs: structuredClone(this.#input), grant, ...(previous ? { grantId: previous.id } : {}), summary: `DM given: ${grant.name} — ${grant.reason}` });
      }, this.#context?.host.locale));
    }
    async #review(request: Omit<Request, "contractVersion">): Promise<void> {
      const invalid = [...this.querySelectorAll<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>("input,select,textarea")].find(input => !input.disabled && !input.validity.valid);
      if (invalid) { invalid.reportValidity(); return; }
      await this.#guard(async () => {
        const response = await this.#call("preview", request); this.#evaluation = response.evaluation; this.#message = response.message;
        const content: Node[] = [el("p", this.#t(response.message || "Review the exact changes below. Saving creates a new retained revision.")), this.#issues()];
        if (response.rulesChanged) content.push(checkbox(this.#t("Adopt the displayed rules and source revision"), request.adoptRules === true, checked => { request.adoptRules = checked; }));
        if (this.#response?.role === "dm" && ["import", "restore"].includes(request.operation ?? "")) content.push(checkbox(this.#t("I authorize the DM grants in this imported or restored character"), request.reauthorizeGrants === true, checked => { request.reauthorizeGrants = checked; }));
        ...
        if (response.token) { const token = response.token, reviewedRequest = JSON.stringify(request); content.push(button(this.#t("Save new revision"), () => this.#guard(async () => {
    ...
    async #history(before?: number): Promise<void> { await this.#guard(async () => { this.#historyResponse = await this.#call("history", { key: this.#key, ...(before ? { before } : {}) }); }); }
    #historyView(): HTMLElement {
      ...
      if (this.#response?.state) root.append(button(this.#t("Export with history"), () => this.#exportHistory()));
      ...
      if ((this.#response?.revision ?? 0) > 1) root.append(button(this.#t("Compare saved revisions"), () => this.#compare()));
      const visible = (this.#historyResponse?.history ?? []).filter(entry => {
        if (this.#historyFilter === "all") return true;
        if (this.#historyFilter === "grant") return entry.operation.includes("grant");
        if (this.#historyFilter === "play") return /play|inventory|spells/u.test(entry.operation);
        return entry.operation.endsWith(this.#historyFilter);
      });
      for (const entry of visible) root.append(panel(this.#t("Revision {0}", [entry.revision]), el("p", this.#t("{0} · {1} · {2}", [entry.summary, entry.actorId, new Date(entry.occurredAt).toLocaleString()])), button(this.#t("Inspect revision"), () => this.#guard(async () => {
        const previous = await this.#call("revision", { key: this.#key, revision: entry.revision }); const state = previous.state!;
      ...
      if (!this.#historyResponse?.history?.length) root.append(el("p", this.#t("No saved revisions yet.")));
      if (this.#historyResponse?.nextBefore) root.append(button(this.#t("Older revisions"), () => this.#history(this.#historyResponse!.nextBefore))); return root;
    }
    #compare(): void {
      ...
      from.required = true; to.required = true;
    ...
    #restore(revision: number, scope = "build"): void {
    ...
    #import(): void {
      let body = this.#transfer?.read() ?? "", authorize = false; const area = textInput(body, value => { body = value; }, true), file = el("input"); file.type = "file"; file.accept = ".json,application/json";
      ...
      this.#open(this.#t("Import current character"), [el("p", this.#t("File and paste use the same validation and replacement review. The current revision will remain in History.")), el("p", this.#t("Save a frozen import on this device when rules are unavailable. Its claimed calculations and external history cannot authorize play. The transfer remains here until you discard it; installation backups do not include device drafts.")), field(this.#t("Choose a file"), file), field(this.#t("Or paste the export"), area), ...(this.#response?.role === "dm" ? [checkbox(this.#t("Authorize imported DM grants as the current DM"), false, value => { authorize = value; })] : []), button(this.#t("Save frozen import on this device"), () => {
        try { this.#transfer?.write(body); this.#message = "Frozen import saved on this device. Reopen Import character to review or export it."; this.#status(); } catch (error) { this.#message = String(error); this.#status(); }
      }), button(this.#t("Inspect external history"), () => {
        try { parseCharacter(body); const archive = externalHistory(body); this.#transfer?.write(body); this.#open(this.#t("External history · unverified provenance"), [el("p", this.#t("These imported actor claims and snapshots are reference material. They never replace local History or grant DM authority.")), ...archive.map(entry => panel(this.#t("Revision {0}", [entry.revision]), el("p", `${entry.summary} · ${entry.actorId} · ${entry.occurredAt}`), el("details", el("summary", this.#t("Imported snapshot")), el("pre", JSON.stringify(entry.state, null, 2))))), button(this.#t("Export original transfer"), () => download(`${this.#key}.transfer.json`, body))]); } catch (error) { this.#message = String(error); this.#status(); }
      }), button(this.#t("Discard device transfer"), () => { this.#transfer?.clear(); body = ""; area.value = ""; }), button(this.#t("Review import"), async () => { try { const inputs = parseCharacter(body); this.#transfer?.write(body); this.#dialog?.close(); await this.#review({ ...this.#base("import"), inputs, reauthorizeGrants: authorize, summary: "Import character replacement" }); } catch (error) { this.#message = error instanceof Error ? error.message : "Invalid character file."; this.#status(); } })]);
    }
    #exportHistory(): void {
      const state = this.#response!.state!, revision = this.#response!.revision;
      let count = Math.min(3, revision);
      const input = numberInput(count, value => { count = value ?? 0; }, 1, Math.min(5, revision)); input.required = true;
      this.#open(this.#t("Export with history"), [el("p", this.#t("Include up to five recent saved snapshots, within the 1 MB transfer limit. The archive is external provenance when imported; full backups retain all local history.")), field(this.#t("Recent revisions"), input), button(this.#t("Download history archive"), () => {
        ...
        return this.#guard(async () => {
          ...
          const archive: ExternalRevision[] = [];
