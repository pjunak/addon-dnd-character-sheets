...
export function printCharacter(state: State, _revision: number, name: string, options: { spells: boolean; equipment: boolean; provenance: boolean }, locale = "en"): void {
  ...
  if (options.equipment && state.inputs.play.containers?.length) root.append(storageRead(state.inputs, locale));
  const containers = containerOptions(state.inputs.play.containers ?? []);
  if (options.equipment && state.inputs.play.inventory.some(item => item.quantity > 0)) root.append(panel(t("Equipment"), ...state.inputs.play.inventory.filter(item => item.quantity > 0).map(item => el("p", t("{0} × {1}; {2}{3}. {4}", [item.quantity, item.name, t(label(item.location)), item.attuned ? t("; attuned") : "", item.notes + (item.containerId ? " · " + t("Container: {0}", [containers.find(container => container.id === item.containerId)?.label ?? item.containerId]) : "")])))));
  if (options.spells) {
