...
export function playActions(input: Inputs, evaluation: Result, catalog: CatalogRecord[], act: (change: Record<string, unknown>, summary: string) => Promise<void>, locale = "en"): HTMLElement {
  ...
  for (const caster of rows(evaluation.spellOptions["classes"])) {
    ...
    for (const [ref, rawSlots] of Object.entries(object(caster["castSlots"]))) {
      ...
      if (slots.length) row.append(field(t("Spend slot"), select(slot, slots.map(id => ({ id, label: id })), value => { slot = value; }, t)));
      row.append(button(t("Cast"), () => act({ operation: "cast-spell", classId, ref, slot }, `Cast ${spellName(ref)}`))); group.append(row);
    }
    for (const ref of strings(caster["ritualIds"])) group.append(el("div", rule(spellName(ref), { kind: "spell", id: ref }), button(t("Cast as ritual"), () => act({ operation: "cast-ritual", classId, ref }, `Ritual: ${spellName(ref)}`))));
  ...
  for (const grant of rows(evaluation.spellOptions["granted"])) {
    ...
    const row = el("div", rule(spellName(ref), { kind: "spell", id: ref }));
    if (slots.length) row.append(field(t("Granted cast resource"), select(slot, slots.map(id => ({ id, label: id })), value => { slot = value; }, t)));
