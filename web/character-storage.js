...
export function storageRead(input, locale) {
    ...
    if (!input.play.containers?.length)
        root.append(el("p", t("No containers yet.")));
    return root;
}
export function storage(view) {
    if (!view.canEditStorage)
        return storageRead(view.input, view.locale);
    const t = translator(view.locale), root = panel(t("Storage containers"));
    root.className = "dse-section dse-storage";
    const heading = root.querySelector("h3");
    heading.tabIndex = -1;
    heading.dataset["focusKey"] = "storage/heading";
    ...
    const add = button(t("Add container"), () => {
        ...
        view.change();
        view.refresh();
        const name = owner?.querySelector('[data-focus-key="storage/' + container.id + '/name"]');
        name?.focus();
        name?.select();
    ...
    for (const container of view.input.play.containers ?? []) {
        const row = styled("div", "dse-storage-group");
        row.dataset["container"] = container.id;
        row.dataset["focusScope"] = "";
        const name = textInput(container.name, value => { container.name = value; view.change(); });
        name.required = true;
        if (Number.isFinite(maximumName))
            name.maxLength = maximumName;
        name.dataset["focusKey"] = "storage/" + container.id + "/name";
        const nameField = field(t("Container name"), name);
        nameField.dataset["uiKey"] = "storage/" + container.id + "/name";
        ...
        const remove = button(t("Remove container"), () => {
            const rows = [...root.querySelectorAll("[data-container]")], index = rows.indexOf(row);
            const next = rows[index + 1] ?? rows[index - 1];
            (next?.querySelector("input") ?? heading).focus();
            if (removeContainer(view.input, container.id)) {
                view.change();
                view.refresh();
            }
        }, false, "storage/" + container.id + "/remove");
        remove.setAttribute("aria-label", t("Remove container: {0}", [container.name]));
        row.append(nameField, el("span", t("Inventory entries: {0}", [count])), remove);
        root.append(row);
    }
    root.append(el("p", t("Removing a container leaves its items in inventory.")));
    return root;
}
export function itemContainer(view, item) {
    ...
    if (!view.canEditStorage)
        return item.containerId ? styled("span", "dse-item-container", t("Container: {0}", [selected?.label ?? item.containerId])) : undefined;
    const control = select(item.containerId ?? "", options.map(option => ({ ...option, disabled: item.location === "equipped" })), value => {
        if (assignContainer(view.input, item.id, value)) {
            view.change();
            view.refresh();
        }
    }, t);
    ...
    wrapper.classList.add("dse-item-container");
    wrapper.dataset["uiKey"] = "inventory/" + item.id + "/container";
    return wrapper;
}
