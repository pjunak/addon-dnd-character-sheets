...
export function projectionView(projection, locale = "en") {
    ...
    const stat = (name, path, value) => el("div", savedRule(name, undefined, projection.explanations[path]), el("strong", human(value)));
    const derived = object(sheet["derived"]), vitals = el("div");
    ...
    for (const ability of abilities) {
        ...
        scores.append(panel(ability, stat(t("Score"), `abilities.${ability}.score`, score["score"]), stat(t("Modifier"), `abilities.${ability}.mod`, score["mod"])));
    }
    root.append(scores);
    for (const group of ["skills", "saves"]) {
        const list = el("dl");
        list.className = "character-values";
        for (const [key, value] of Object.entries(object(sheet[group]))) {
            list.append(el("dt", savedRule(t(label(key)), group === "skills" ? { kind: "skill", id: key } : undefined, projection.explanations[`${group}.${key}.total`])), el("dd", human(object(value)["total"])));
        }
        root.append(panel(t(label(group)), list));
    }
    const senses = el("dl");
    senses.className = "character-values";
    for (const [key, value] of Object.entries(object(sheet["senses"]))) {
        const explanation = projection.explanations[`senses.${key}`];
        senses.append(el("dt", savedRule(t(label(key)), undefined, explanation)), el("dd", t("{0} {1}", [human(value), explanation?.unit ?? ""])));
    }
    ...
    for (const group of ["weapons", "spellcasting", "resources", "features"]) {
        ...
        rows(group === "spellcasting" ? object(sheet[group])["perClass"] : sheet[group]).forEach((row, index) => {
            ...
            if (evidence?.summary && group === "features")
                details.append(el("p", evidence.summary));
            const values = el("dl");
            for (const [key, value] of Object.entries(row)) {
                if (["id", "name", "key", "kind", "classId", "source", "ref", "text", "description"].includes(key))
                    continue;
                const path = group === "resources" ? `resources.${String(row["key"])}.${key}` : `${prefix}.${index}.${key}`;
                values.append(el("dt", savedRule(t(label(key)), undefined, projection.explanations[path])), el("dd", key === "recharge" ? rows(value).map(entry => t("{0} rest: {1}", [t(label(String(entry["on"]))), entry["amount"] === "full" ? t("all uses") : human(entry["amount"])])).join("; ") : human(value)));
            }
    ...
    if (active.length)
        root.append(panel(t("Active conditions"), ...active.map(row => el("p", t("{0}: {1}", [human(row["name"]), human(row["condition"])])))));
    return root;
}
export function printCharacter(state, revision, name, options, locale = "en") {
    const t = translator(locale);
    ...
    if (options.provenance) {
        ...
        root.append(panel(t("Build decisions"), el("p", `${recordName("species", build.species)} · ${recordName("background", build.background)} · ${t(label(build.method))}`), el("p", human(build.baseScores)), ...build.levels.map((level, index) => el("p", t("Level {0}: {1}", [index + 1, recordName("class", level.classId)]))), ...build.choices.map(choice => el("p", `${choice.id} (${choice.slot + 1}): ${human(choice.value)}`))), panel(t("Rules revision"), el("p", human(state.rules))));
        if (build.spells.acquisitions.length)
            root.append(panel(t("Spell acquisitions"), ...build.spells.acquisitions.map(entry => el("p", `${recordName("spell", entry.spellId)} · ${entry.costGp} GP · ${entry.origin === "import" ? t("Imported claim") : t("Recorded")}`))));
        root.append(panel(t("Sources"), ...state.projection.evidence.map(source => el("p", t("{0} ({1}:{2}), {3}, {4}", [source.name, source.reference.kind, source.reference.id, source.book ?? "", source.hash]), ...(source.packageId ? [el("small", `${t("Source package")}: ${source.packageId} · ${source.packageGeneration ?? ""}`)] : [])))));
    }
    // Print is frozen text from the selected revision; no provider is contacted.
    for (const details of root.querySelectorAll("details"))
        details.open = true;
    for (const component of root.querySelectorAll("codex-addon-rule-details")) {
        const data = component.details;
        component.replaceWith(el("span", data.label));
    }
    view.document.body.append(view.document.importNode(root, true));
    view.focus();
    view.print();
}
