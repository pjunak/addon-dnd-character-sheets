...
export function grantForm(input, evaluation, feats, previous, submit, locale = "en") {
    ...
    const renderEffects = () => {
        ...
        for (const [index, effect] of grant.effects.entries()) {
            ...
            list.append(panel(t("Effect {0}", [index + 1]), field(t("Effect"), select(effect.target, ["abilityScore", "abilityCap", "armorClass", "savingThrow", "initiative", "speed", "maxHp", "attunementLimit", "sense", "resourceMax", "proficiency"].map(id => ({ id, label: t(label(id)) })), value => { effect.target = value; delete effect.key; renderEffects(); }, t)), ...(keys.length ? [field(t("Applies to"), select(effect.key ?? "", keys.map(id => ({ id, label: t(label(id)) })), value => { effect.key = value; }, t))] : []), field(t("Operation"), select(effect.mode, ["add", "set", "minimum", "maximum"].map(id => ({ id, label: t(label(id)) })), value => { effect.mode = value; }, t)), field(t("Amount"), numberInput(effect.value, value => { effect.value = value ?? 0; })), button(t("Remove effect"), () => { grant.effects.splice(index, 1); renderEffects(); })));
        }
    };
    effects.append(el("p", t("Leave empty for a narrative reward. Typed effects participate in calculation and can be reversed.")), list, button(t("Add effect"), () => { grant.effects.push({ target: "maxHp", mode: "add", value: 0 }); renderEffects(); }));
    renderEffects();
    ...
    return [field(t("Name"), name), field(t(previous ? "Reason for amendment" : "Reason"), reason), field(t("Effective character level"), numberInput(grant.effectiveLevel, value => { grant.effectiveLevel = value ?? 1; }, 1)), effects,
        field(t("Granted feat (optional)"), select(grant.feat?.id ?? "", feats.map(record => ({ id: record.id, label: String(record.value["name"] ?? record.id) })), value => { if (value)
        ...
        button(t("Review DM grant"), () => { if (!name.reportValidity() || !reason.reportValidity())
            return; return submit(grant); })];
}
