...
export function pinQuickUse(input, id, pinned) {
    ...
    if (current.includes(id) === pinned)
        return false;
    const next = pinned ? [...current, id] : current.filter(itemId => itemId !== id);
    if (next.length)
        input.play.quickUse = next;
    else
        delete input.play.quickUse;
    return true;
}
export function removeInventoryItem(input, id) {
...
export function attunementChoice(item, guidance) {
    ...
    if (item.location !== "equipped")
        return { allowed: false, reason: "not-equipped" };
    return { allowed: true };
}
export function attuneEquipment(item, attuned, guidance) {
    ...
    if (attuned && !attunementChoice(item, guidance).allowed)
        return false;
    item.attuned = attuned;
    return true;
}
export function stowAndUnattune(item) {
    if (!item.attuned)
        return false;
