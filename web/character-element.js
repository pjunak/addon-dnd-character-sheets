...
export function defineCharacterElement(generation, client, enhance) {
    ...
    class CharacterElement extends HTMLElement {
        ...
        #syncBusyButtons() {
            for (const control of this.querySelectorAll("button,input,select,textarea")) {
        ...
        async #flush() {
            ...
            const run = async () => {
                while (this.#dirty && !this.#blocked && this.isConnected && epoch === this.#epoch) {
                    ...
                    if (response.status === "conflict" && response.state) {
                        ...
                        this.#blocked = true;
                        this.#message = "This character was edited elsewhere. Your pending input remains on this page. Reload the character before continuing.";
                        this.#status();
                        break;
                    }
                    ...
                    if (this.#input.play.asOf === inputs.play.asOf)
                        this.#input.play.asOf = response.state.inputs.play.asOf;
                    if (this.#input.play.hp === inputs.play.hp)
                        this.#input.play.hp = response.state.inputs.play.hp;
                    ...
                    if (version === this.#changeVersion) {
                        ...
                        if (focused instanceof HTMLTextAreaElement || focused instanceof HTMLInputElement && focused.type === "text" && focused.getAttribute("role") !== "combobox") {
                            ...
                            this.#input = structuredClone(response.state.inputs);
                            this.#render();
                        }
                    }
                }
            ...
            this.#saving = run().catch(error => {
                if (epoch === this.#epoch && this.isConnected) {
                    this.#blocked = true;
                    this.#message = error instanceof Error ? error.message : "Could not save. Check your connection and retry.";
                    this.#render();
                }
            }).finally(() => { if (epoch === this.#epoch) {
                this.#saving = undefined;
                this.#publish();
                if (!this.#dirty && !this.#evaluation)
                    void this.#refreshSaved(true);
            } });
            this.#publish();
            return this.#saving;
        }
        ...
        #render() {
            ...
            for (const field of root.querySelectorAll(".character-field")) {
                ...
                if (field.dataset["uiKey"] === field.querySelector("label")?.textContent)
        ...
        #spellChoices() {
            ...
            for (const choice of rows(options["pendingChoices"])) {
                ...
                root.append(picks(spellSourceLabel(choice["source"], this.#context?.host.locale) + " · " + this.#t("Granted spells · choose {0}", [choice["choose"]]), strings(choice["eligibleSpellIds"]), this.#input.build.spells.grantChoices[key] ?? [], value => { this.#input.build.spells.grantChoices[key] = value; }, Number(choice["choose"]), key));
            }
            for (const choice of rows(options["castingAbilityChoices"])) {
                const key = String(choice["key"]);
                root.append(builderTarget(key, field(spellSourceLabel(choice["source"], this.#context?.host.locale) + " · " + this.#t("Granted spellcasting ability"), select(this.#input.build.spells.castingAbilities[key] ?? "", strings(choice["options"]).map(id => ({ id, label: id })), value => { this.#input.build.spells.castingAbilities[key] = value; this.#changed(); }, this.#t))));
            }
            return root;
        }
        #grants() {
            ...
            for (const grant of this.#response?.state?.inputs.grants ?? []) {
                ...
                if (grant.active && this.#response?.role === "dm")
                    root.append(button(this.#t("Amend {0}", [grant.name]), () => this.#grant(grant), !this.#editable), button(this.#t("Revoke {0}", [grant.name]), () => this.#perform({ ...this.#base("revoke-grant"), grantId: grant.id, inputs: structuredClone(this.#input), summary: `Revoke DM grant: ${grant.name}` }), !this.#editable));
            }
        ...
        #grant(previous) {
            this.#open(previous ? this.#t("Amend {0}", [previous.name]) : this.#t("DM given"), grantForm(this.#input, this.#evaluation, this.#catalogs.get("feat") ?? [], previous, grant => {
                this.#dialog?.close();
                return this.#perform({ ...this.#base(previous ? "amend-grant" : "grant"), inputs: structuredClone(this.#input), grant, ...(previous ? { grantId: previous.id } : {}), summary: `DM given: ${grant.name} — ${grant.reason}` });
