...
export function buildView(view, active = "character") {
    ...
    const update = (action, render = false) => { action(); view.changed(); refreshSteppers(root); if (render)
        view.refresh(); };
    if (active === "character") {
        const foundation = panel(t("Character"));
        ...
        const method = select(build.method, ["point-buy", "array", "rolled"].map(id => ({ id, label: t(label(id)) })), value => {
            ...
            update(() => {
                ...
                build.rolls = [];
                build.baseScores = {};
                if (value === "point-buy" && Number.isFinite(minimum))
                    for (const ability of abilities)
                        build.baseScores[ability] = minimum;
                if (value === "array")
                    abilities.forEach((ability, index) => { build.baseScores[ability] = Number(policy["standardArray"][index]); });
            }, true);
        }, t);
        ...
        const spent = () => abilities.reduce((sum, ability) => sum + Number(costs[String(build.baseScores[ability] ?? minimum)] ?? 0), 0);
        const progress = styled("p", "dnd-builder-progress");
        progress.setAttribute("role", "status");
        const refreshBudget = () => { progress.textContent = t("{0} / {1} points used · {2} remaining", [spent(), budget, budget - spent()]); };
        for (const ability of abilities) {
            if (build.method === "point-buy" && Number.isFinite(minimum) && Number.isFinite(maximum)) {
                const limit = () => { const current = build.baseScores[ability] ?? minimum; let allowed = current; for (let next = current + 1; next <= maximum; next++)
                    if (spent() - Number(costs[String(current)]) + Number(costs[String(next)]) <= budget)
                        allowed = next; return allowed; };
                scores.append(field(ability, stepper(() => build.baseScores[ability] ?? minimum, value => update(() => { for (const key of abilities)
                    build.baseScores[key] ??= minimum; build.baseScores[ability] = value; refreshBudget(); }), minimum, limit)));
            }
            else if (build.method === "array") {
                const array = (policy["standardArray"] ?? []);
                scores.append(field(ability, select(String(build.baseScores[ability] ?? ""), [...new Set(array)].map(score => ({ id: String(score), label: String(score) })), value => {
                    if (!value)
                        return;
                    update(() => { const other = abilities.find(key => key !== ability && build.baseScores[key] === Number(value)); if (other) {
                        const prior = build.baseScores[ability];
                        if (prior === undefined)
                            delete build.baseScores[other];
                        else
                            build.baseScores[other] = prior;
                    } build.baseScores[ability] = Number(value); }, true);
                }, t)));
            }
                ...
                dice.forEach((_, index) => group.append(field(t("Die {0}", [index + 1]), stepper(() => dice[index], value => update(() => {
                    ...
                    build.rolls.push({ id: newId(), ability, dice: [...dice], kept });
                    build.baseScores[ability] = kept.reduce((sum, index) => sum + dice[index], 0);
                    result.textContent = String(build.baseScores[ability]);
        ...
        if (build.method === "point-buy" && Number.isFinite(budget)) {
            refreshBudget();
            foundation.append(progress);
        }
        for (const kind of ["species", "background"]) {
            const records = view.catalogs.get(kind) ?? [], choice = combo(build[kind], options(records), value => update(() => { build[kind] = value; if (kind === "species")
                build.lineage = ""; }, true), t);
        ...
        for (const repair of unassignedFeatChoices(view, rows(plan["creationChoices"]), guidance))
    ...
    const remove = (index) => update(() => {
        ...
        for (const descriptor of rows(plan["classChoices"]))
            if (Number(object(descriptor["source"])["level"] ?? descriptor["level"] ?? 1) > (counts.get(String(descriptor["classId"])) ?? 0)) {
                const id = String(descriptor["id"]);
                build.choices = build.choices.filter(choice => choice.id !== id && !choice.id.startsWith(id + ":"));
            }
        for (const id of Object.keys(build.subclasses))
            if (!counts.has(id) || (counts.get(id) ?? 0) < Number(classes.find(row => row["classId"] === id)?.["subclassLevel"]))
                delete build.subclasses[id];
        for (const selections of [build.spells.cantrips, build.spells.spellbook, input.play.preparedSpells])
    ...
    build.levels.forEach((level, index) => {
        if (active !== "levels" && level.classId !== active)
            return;
        classLevel++;
        ...
        if (active !== "levels") {
            ...
            if (index > 0) {
                row.append(field(t("HP gain"), select(level.hitPoints === undefined ? "fixed" : "rolled", [{ id: "fixed", label: t("Fixed") }, { id: "rolled", label: t("Recorded roll") }], value => update(() => { if (value === "rolled")
                    ...
                    delete level.hitPoints; }, true), t)));
                if (level.hitPoints !== undefined)
                    row.append(field(t("Recorded hit-die roll"), stepper(() => level.hitPoints, value => update(() => { level.hitPoints = value; }), 1, Number(currentClass?.["hitDieMax"]))));
            }
        }
        row.append(button(t("Remove level"), () => remove(index), false));
        levels.append(row);
    });
    if (active !== "levels")
        levels.append(button(t("Add level"), () => add(active), !available.some(option => option.id === active)));
    if (!build.levels.length)
        levels.append(el("p", t("Use + to choose your first class.")));
    root.append(levels);
    return root;
}
function unassignedFeatChoices(view, descriptors, guidance) {
    const t = translator(view.locale), result = [], aliases = new Set(descriptors.map(row => String(row["legacyId"] ?? "")).filter(Boolean));
    for (const alias of aliases) {
        if (!view.input.build.choices.some(choice => choice.id === alias))
            continue;
        const owners = descriptors.filter(row => row["legacyId"] === alias);
        const repair = panel(t("Assign saved feat choices"));
        repair.id = `character-choice-${encodeURIComponent(alias)}`;
...
function choiceView(descriptor, guidance, view) {
    ...
    const set = (slot, value) => { view.input.build.choices = view.input.build.choices.filter(choice => choice.id !== id || choice.slot !== slot); if (value !== "")
        view.input.build.choices.push({ id, slot, value }); view.changed(); refreshSteppers(root); };
    if (kind === "abilityBudget" || descriptor["budget"] !== undefined) {
        ...
        const spent = () => Object.values(object(current(0))).reduce((sum, value) => sum + Number(value), 0), progress = el("p");
        const refresh = () => { progress.textContent = t("{0} / {1} points used · {2} remaining", [spent(), budget, budget - spent()]); };
        refresh();
        root.append(progress);
        const scores = styled("div", "character-stats");
        for (const ability of eligible)
            scores.append(field(ability, stepper(() => Number(object(current(0))[ability] ?? 0), value => { set(0, { ...object(current(0)), [ability]: value }); refresh(); }, 0, () => Math.min(limit, budget - spent() + Number(object(current(0))[ability] ?? 0)))));
        root.append(scores);
    }
    else if (kind === "asiMode") {
        const mode = String(current(0) ?? "");
        root.append(field(t("Advancement"), select(mode, [{ id: "asi", label: t("Ability increases") }, { id: "feat", label: t("Feat") }], value => { view.input.build.choices = view.input.build.choices.filter(choice => !choice.id.startsWith(id + ":")); set(0, value); view.refresh(); }, t)));
        if (mode === "asi")
