...
import { button, el, field, human, numberInput, panel, rule, select } from "./character-ui.js";
// Options and costs are engine results. This view only gathers command inputs.
export function playActions(input, evaluation, catalog, act, locale = "en") {
    const t = translator(locale);
    const root = panel(t("Spellcasting and recovery")), names = new Map(catalog.map(record => [record.id, String(record.value["name"] ?? record.id)]));
    ...
    for (const caster of rows(evaluation.spellOptions["classes"])) {
        const classId = String(caster["classId"]), group = panel(t("{0} spells", [classId]));
        for (const [ref, rawSlots] of Object.entries(object(caster["castSlots"]))) {
            ...
            if (slots.length)
                row.append(field(t("Spend slot"), select(slot, slots.map(id => ({ id, label: id })), value => { slot = value; }, t)));
        ...
        for (const ref of strings(caster["ritualIds"]))
            group.append(el("div", rule(spellName(ref), { kind: "spell", id: ref }), button(t("Cast as ritual"), () => act({ operation: "cast-ritual", classId, ref }, `Ritual: ${spellName(ref)}`))));
        ...
        if (copyOptions.length) {
            let ref = "", scrollId = "";
            group.append(field(t("Spell to copy"), select("", copyOptions.map(id => ({ id, label: t("{0} · {1} GP", [spellName(id), human(costs[id])]) })), value => { ref = value; }, t)), field(t("Owned scroll (optional)"), select("", input.play.inventory.filter(item => item.spellId && item.quantity > 0).map(item => ({ id: item.id, label: item.name })), value => { scrollId = value; }, t)), button(t("Review copying cost"), async () => { if (ref)
        ...
        if (caster["canSwap"] === true && input.play.preparedSpells[classId]?.length) {
            let out = "", ref = "";
            group.append(field(t("Replace known spell"), select("", input.play.preparedSpells[classId].map(id => ({ id, label: spellName(id) })), value => { out = value; }, t)), field(t("Replacement spell"), select("", strings(caster["spellIds"]).filter(id => Number(catalog.find(record => record.id === id)?.value["level"]) > 0).map(id => ({ id, label: spellName(id) })), value => { ref = value; }, t)), button(t("Review spell replacement"), async () => { if (out && ref)
    ...
    for (const grant of rows(evaluation.spellOptions["granted"])) {
        ...
        const row = el("div", rule(spellName(ref), { kind: "spell", id: ref }));
        if (slots.length)
            row.append(field(t("Granted cast resource"), select(slot, slots.map(id => ({ id, label: id })), value => { slot = value; }, t)));
