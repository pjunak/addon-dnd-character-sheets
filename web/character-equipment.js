...
export function equipmentPicker(catalogs, locale, submit, containers = []) {
    ...
    const addItems = (items) => submit(items.map(item => destination ? { ...item, containerId: destination } : item));
    ...
    const path = styled("nav", "dnd-equipment-path");
    ...
    const categoryOf = (record) => String(record.value["category"] ?? record.value["type"] ?? t("Other"));
    const render = () => {
        path.replaceChildren(button(t("All equipment"), () => { category = ""; folder = ""; render(); }));
        if (category)
            path.append(el("span", "›"), button(t(label(category)), () => { folder = ""; render(); }));
        if (folder)
            path.append(el("span", "›"), el("span", t(label(folder))));
        ...
        if (!query && (!category || !folder)) {
            ...
            for (const id of folders) {
                const next = button("▸ " + t(label(id)), () => { if (!category)
                    category = id;
                else
                    folder = id; render(); });
                next.className = "dnd-equipment-folder";
                results.append(next);
            }
        }
        ...
        if ((query || folder) && !visible.length)
            results.append(el("p", t("No matching items.")));
        if (visible.length > 200)
            results.append(el("p", t("Refine your search to see more items.")));
        for (const [key, entry] of tray) {
            ...
            const remove = button("×", () => { tray.delete(key); render(); });
            remove.setAttribute("aria-label", t("Remove {0}", [name]));
            selected.append(styled("div", "dnd-picker-row", el("span", name), quantity, remove));
        }
        if (!tray.size)
            selected.append(styled("p", "dse-empty", t("Choose items to add to your backpack.")));
        selected.append(button(t("Add selected items"), () => {
            if (![...root.querySelectorAll("input")].every(input => input.reportValidity()))
                return;
    ...
    const search = textInput("", value => { query = value.toLocaleLowerCase().trim(); render(); });
    search.setAttribute("type", "search");
    root.append(field(t("Find catalog item"), search), path, styled("div", "dnd-picker-split", results, selected));
    const custom = el("details", el("summary", t("Add narrative item")));
    let name = "", quantity = 1;
    custom.append(field(t("Name"), textInput("", value => { name = value; })), field(t("Quantity"), numberInput(1, value => { quantity = value ?? 1; }, 1)), button(t("Add narrative item"), () => {
        if (name.trim() && quantity > 0)
            addItems([{ id: newId(), name: name.trim(), quantity, location: "carried", attuned: false, acquisition: "", notes: "" }]);
    }));
    root.append(custom);
    render();
    return root;
